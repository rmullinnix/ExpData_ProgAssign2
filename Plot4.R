...
scc_id <- class.df[grep("coal", class.df$Short.Name, ignore.case=TRUE), "SCC"]

yr_sum <- tbl_df(summary.df)
coal_filt <- filter(yr_sum, SCC %in% scc_id)
by_year <- group_by(coal_filt, year)
total_by_year <- summarise(by_year, arr=sum(Emissions, na.rm=TRUE))

png("plot4.png")

plot(total_by_year$year, total_by_year$arr, type="b",
     main=expression('US Total Coal Emissions from PM'[2.5]),
     xlab="Year",
